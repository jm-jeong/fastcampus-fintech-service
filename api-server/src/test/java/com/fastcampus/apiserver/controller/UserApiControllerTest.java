...
class UserApiControllerTest {
	...
	public void 회원가입시_같은_이메일로_회원가입하면_에러발생() throws Exception {
		...
		when(userService.join(userRegisterRequest)).thenThrow(new ApiException(UserErrorCode.DUPLICATED_USER_NAME));

		mockMvc.perform(post("/api/v1/users/join")
				.contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsBytes(new UserRegisterRequest("nameaa", email, "password1234"))))
			.andDo(print())
			.andExpect(status().is(UserErrorCode.DUPLICATED_USER_NAME.getHttpStatusCode()));
	}
